  ...
  <div class="display-inline-block width-100 margin-45px-bottom xs-margin-25px-bottom">
     <form name="search"action=""method="post" enctype="multipart/form-data">
      <div class="position-relative">
        <input type="text" name="s" class="bg-transparent text-small no-margin border-color-extra-light-gray medium-input pull-left" placeholder="Pesquisar artigos..." autocomplete="off">
        <button class="bg-transparent btn position-absolute right-0 top-1"><i class="fa fa-search no-margin-left"></i></button>
      </div>   
    </form>
  </div>
<!--BANNER CTA-->
  ...
  <div class="margin-45px-bottom xs-margin-25px-bottom">
    <div class="text-extra-dark-gray margin-20px-bottom alt-font text-uppercase text-small font-weight-600 aside-title"><span>Sobre a empresa</span></div>
    <a href="about-me.html"><img src="<?= INCLUDE_PATH; ?>/images/doripel-fabrica-aerea-2.jpg" alt="Móveis Doripel Indústria Moveleira - Vista Aérea" class="margin-25px-bottom"/></a>
    <p class="margin-20px-bottom text-small">A Doripel conta com processos de fabricação modernos que resulta em produtos de alto padrão e qualidade e uma ótima relação custo/benefício.</p>
    <a class="btn btn-very-small btn-dark-gray text-uppercase" href="<?= BASE; ?>/sobre">Sobre a Doripel</a>
  </div>
  <div class="margin-50px-bottom">
    <div class="text-extra-dark-gray margin-20px-bottom alt-font text-uppercase font-weight-600 text-small aside-title"><span>Siga-nos</span></div>
  ...
  <div class="margin-45px-bottom xs-margin-25px-bottom">
    <div class="text-extra-dark-gray margin-25px-bottom alt-font text-uppercase font-weight-600 text-small aside-title"><span>Mais lidos</span></div>
    <ul class="latest-post position-relative">
      ...
      if (!$Read->getResult()):
        ...
        foreach ($Read->getResult() as $Post):
          ...
          <li>
            <figure>
              <a title="Ler mais sobre <?= $Post['post_title']; ?>" href="<?= BASE; ?>/artigo/<?= $Post['post_name']; ?>"><img title="<?= $Post['post_title']; ?>" alt="<?= $Post['post_title']; ?>" src="<?= BASE; ?>/tim.php?src=uploads/<?= $Post['post_cover']; ?>&w=<?= IMAGE_W / 2; ?>&h=<?= IMAGE_H / 2; ?>"/></a>
            </figure>
            <div class="display-table-cell vertical-align-top text-small"><a title="Ler mais sobre <?= $Post['post_title']; ?>" href="<?= BASE; ?>/artigo/<?= $Post['post_name']; ?>" class="text-extra-dark-gray"><span class="display-inline-block margin-5px-bottom"><?= $Post['post_title']; ?></span></a> <span class="clearfix text-medium-gray text-small"><time datetime="<?= date('Y-m-d', strtotime($Post['post_date'])); ?>" pubdate="pubdate"><?= utf8_encode(strftime(" %d de %B de %Y", strtotime($Post['post_date']))); ?></time></span>
            </div>
          </li>
          <?php
        endforeach;
  ...
  <div class="margin-45px-bottom xs-margin-25px-bottom">
    <div class="text-extra-dark-gray margin-25px-bottom alt-font text-uppercase font-weight-600 text-small aside-title"><span>Arquivo</span></div>
    <ul class="list-style-6 margin-20px-bottom text-small">
      ...
      if ($Read->getResult()):
        ...
        foreach ($Read->getResult() as $MesAno):       
            ...
            $Print = "<li><a href='{$Pesquisa}'>" . getWcMonths($MesAno['post_month']) . "</a></li>";
