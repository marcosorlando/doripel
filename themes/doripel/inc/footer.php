		...
		<div class="container">
			<div class="row equalize xs-equalize-auto">
				...
				<div class="col-md-4 col-sm-2 col-xs-12 text-center display-table xs-margin-10px-bottom">
					<div class="display-table-cell vertical-align-middle">
						<a href="<?= BASE; ?>" title="Voltar ao Início"><img class="footer-logo"
						                                                     src="<?= INCLUDE_PATH; ?>/images/logo-full-white.png"
						                                                     data-at2x="<?= INCLUDE_PATH; ?>/images/logo-full-white@2x.png"
						                                                     alt="Doripel Móveis - logotipo"></a>
					</div>
				</div>
				<!-- end logo -->
				<!-- start social media -->
		...
		<div class="container">
			<div class="row">
				...
				<div class="col-md-3 col-sm-6 col-xs-12 widget sm-margin-30px-bottom xs-text-center">
					...
					<p class="text-small width-95 xs-width-100 no-margin">Visando sempre a satisfação dos clientes, a
						Doripel
						conta com processos de fabricação com tecnologia moderna que resulta em produtos de alto padrão
						e qualidade
						e uma ótima relação custo/benefício, preço acessível e design moderno em um único produto.</p>
					<a href="<?= BASE; ?>/sobre" title="" class="btn btn-very-small btn-white margin-10px-top">Continue
						lendo!</a>
				</div>
				<!-- end about -->
				<!-- start blog post -->
				<div class="col-md-3 col-sm-6 col-xs-12 widget sm-margin-30px-bottom">
					...
					<ul class="latest-post position-relative top-3">
                        <?php
                            $Read ??= new Read();
                            $Read->fullRead(
                                "SELECT p.post_title, p.post_subtitle, p.post_name, p.post_cover, p.post_date, p.post_author, u.user_name, u.user_lastname, u.user_genre FROM " . DB_POSTS . " p, " . DB_USERS . " u WHERE post_status = 1 AND post_date <= NOW() AND post_author = user_id ORDER BY post_date DESC LIMIT :limit",
                                "limit=3"
                            );
                            ...
                            if (!$Read->getResult()) {
                                //              $Pager->returnPage();
                                echo Check::erro(
                                    "Ainda não existem posts cadastrados nesta seção. Favor volte mais tarde.",
                                    E_USER_NOTICE
                                );
                            } else {
				...
				<div class="col-md-3 col-sm-6 col-xs-12 widget sm-margin-30px-bottom xs-text-center">
					...
					<form name="lead_capture" class="j_formsubmit form_lead" action="" method="post"
	...
	<div class="container">
		<div class="footer-bottom border-top border-color-light-gray padding-30px-top">
			<div class="row">
				...
				<div class="col-md-6 col-sm-6 col-xs-12 text-right text-small xs-text-center">&COPY; <?= Date('Y'); ?>
					Doripel
					Móveis é produzido por
					<a href="https://zen.ppg.br" title="Zen Agência Web - Marketing Digital"
					   target="_blank">ZEN AGÊNCIA WEB</a>
				</div>
