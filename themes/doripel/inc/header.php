		...
		<div class="container-fluid nav-header-container">
			<div class="row">
				...
				<div class="col-lg-2 col-md-2 col-xs-5">
					<a href="<?= BASE; ?>" title="Doripel Móveis" class="logo"><img
								src="<?= INCLUDE_PATH; ?>/images/logo.png"
								data-at2x="<?= INCLUDE_PATH; ?>/images/logo@2x.png" class="logo-dark default"
								alt="Doripel Móveis"><img src="<?= INCLUDE_PATH; ?>/images/logo-full-white.png"
					                                      data-at2x="<?= INCLUDE_PATH; ?>/images/logo-full-white@2x.png"
					                                      alt="Doripel Móveis" class="logo-light"></a>
				</div>
				<!-- end logo -->
				<div class="col-lg-9 col-md-9 col-xs-2 accordion-menu">
					...
					<div class="navbar-collapse collapse no-padding-right" id="navbar-collapse-toggle-1">
						<ul id="accordion" class="nav navbar-nav navbar-center alt-font text-normal" data-in="fadeIn"
							...
							<!--start menu item-->
							<li>
								<a title="<?= SITE_NAME; ?> | Home" href="<?= BASE; ?>"><i
											style="font-size:1.5em; padding-left: 8px;" class="fa fa-home"></i></a>
							</li>

                            <?php

                                $Read->fullRead(
                                    "SELECT cat_id, cat_title, cat_name FROM " . DB_PDT_CATS_DORIPEL . " WHERE cat_parent IS NULL AND cat_id IN(SELECT pdt_category FROM " . DB_PDT_DORIPEL . " WHERE pdt_status = 1) ORDER BY cat_title ASC"
                                );
                                ...
                                if ($Read->getResult()) {
                                    foreach ($Read->getResult() as $Cat) {
                                        ...
                                        $Read->exeRead(
                                            DB_PDT_CATS_DORIPEL,
                                            "WHERE cat_parent = :ct AND EXISTS(SELECT 1 FROM " . DB_PDT_DORIPEL . " p WHERE p.pdt_status = 1 AND p.pdt_created <= NOW() AND FIND_IN_SET(cat_id, p.pdt_subcategory)) ORDER BY cat_name ASC",
                                            "ct={$Cat['cat_id']}"
                                        );
                                        if ($Read->getResult()) {
                                ...
                                //if (ACC_MANAGER) {
                                //   echo "<li class='login'>";
                                // require 'assets/widgets/account/account.bar.php';
                                // echo "</li>";
                                //}
                            ?>
							<!--              <li>-->
							...
							<li>
								<a title="<?= SITE_NAME; ?> | PEDIDOS ONLINE"
								   href="http://200.215.160.7:8085/pedidos4/login.asp" target="_blank">
									<span class="btn_pedidos"><i style="font-size:1.2em;"
									                             class="fa fa-file-powerpoint-o animated  infinite"> </i>  PEDIDOS</span>
								</a>
							</li>
				...
				<div class="col-lg-1 col-md-1 col-xs-5 sm-width-auto text-right sm-no-padding-left">
					<div class="header-searchbar no-border-left">
						...
						<!-- search input-->
						<form id="search-header" method="post" action="" name="search"
						      class="mfp-hide search-form-result">
							<div class="search-form position-relative">
								<button class="fa fa-search close-search search-button"></button>
								<input type="text" name="p" class="search-input" placeholder="Digite sua pesquisa..."
								       autocomplete="on">
							</div>
						</form>
					</div>
