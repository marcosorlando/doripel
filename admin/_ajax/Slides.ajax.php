...
if ($PostData && $PostData['callback_action'] && $PostData['callback'] == $CallBack):
    ...
    switch ($Case):
        ...
        case 'manager':
            ...
            $MobileImage = (!empty($_FILES['mobile_image']) ? $_FILES['mobile_image'] : null);
            ...
            elseif (in_array('', $PostData)):
                ...
                if (!empty($Image)):
                 ...
                 if (!empty($MobileImage)):
                    if ($Read->getResult() && !empty($Read->getResult()[0]['mobile_image']) && file_exists("../../uploads/{$Read->getResult()[0]['mobile_image']}") && !is_dir("../../uploads/{$Read->getResult()[0]['mobile_image']}")):
                        unlink("../../uploads/{$Read->getResult()[0]['mobile_image']}");
                    endif;
                    $Upload = new Upload('../../uploads/');
                    $Upload->Image($MobileImage, Check::Name($PostData['slide_title']), '640', 'slides');
                    $PostData['mobile_image'] = $Upload->getResult();
                endif;

                $Update->ExeUpdate(DB_SLIDES, $PostData, "WHERE slide_id = :id", "id={$SlideId}");
                $jSON['trigger'] = AjaxErro("<b class='icon-checkmark'>Tudo certo {$_SESSION['userLogin']['user_name']}</b>: O conteúdo em destaque foi atualizado com sucesso. E será exibido no intervalo de datas cadastradas!");
            endif;
            break;
