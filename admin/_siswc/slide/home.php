...
<div class="dashboard_content">
    ...
    if (!$Read->getResult()):
        $Pager->returnPage();
        echo Check::erro("<span class='al_center icon-notification'>Ainda não existe conteúdo em destaque cadastrado em seu site. Comece cadastrando o primeiro!</span>", E_USER_NOTICE);
    else:
        foreach ($Read->getResult() as $Slide):
            extract($Slide);
            echo "<article class='box box100 slide_single' id='{$slide_id}'>
                    <header>
                        <h1><a target='_blank' href='" . BASE . "/{$slide_content}' title='{$slide_title}'>{$slide_title}</a></h1>
                    </header>
