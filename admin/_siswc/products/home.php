...
if (DB_AUTO_TRASH):
    $Delete = new Delete;
    $Delete->exeDelete(DB_PDT_DORIPEL, "WHERE pdt_title IS NULL AND pdt_content IS NULL and pdt_status = :st", "st=0");

    //AUTO TRASH IMAGES
    $Read->fullRead("SELECT image FROM " . DB_PDT_IMAGE_DORIPEL . " WHERE product_id NOT IN(SELECT pdt_id FROM " . DB_PDT_DORIPEL . ")");
    if ($Read->getResult()):
        $Delete->exeDelete(DB_PDT_IMAGE_DORIPEL,
          "WHERE id >= :id AND product_id NOT IN(SELECT pdt_id FROM " . DB_PDT_DORIPEL . ")", "id=1");
...
<div class="dashboard_content">
    <?php
    if (!$ProductsResult):
        echo Check::erro("<span class='al_center icon-notification'>Ainda não existem produtos cadastrados {$Admin['user_name']}. Comece agora mesmo criando seu primeiro produto!</span>",
          E_USER_NOTICE);
    else:
...
//                echo "<p class='tagline'><span class='offer'>de <strike>R$ " . number_format($pdt_price, "2", ",",
//                    ".") . "</strike> por</span>R$ " . number_format($pdt_offer_price, "2", ",", ".") . "</p>";
//            else:
//                echo "<p class='tagline'><span class='offer'>por apenas</span>R$ " . number_format($pdt_price, "2", ",",
//                    ".") . "</p>";
//            endif;

            $Read->fullRead("SELECT brand_title FROM " . DB_PDT_BRANDS_DORIPEL . " WHERE brand_id = :bid",
              "bid={$pdt_brand}");
            $Brand = ($Read->getResult() ? $Read->getResult()[0]['brand_title'] : 'indefinida');

            $Read->fullRead("SELECT cat_title FROM " . DB_PDT_CATS_DORIPEL . " WHERE cat_id = :cat",
              "cat={$pdt_category}");
            ...
            $PdtSoldVar = null;
            $PdtStockVar = null;
            $Read->fullRead("SELECT stock_code, stock_inventory, stock_sold FROM " . DB_PDT_STOCK_DORIPEL . " WHERE pdt_id = :id",
              "id={$pdt_id}");
            if ($Read->getResult()):
...
//                  <!-- <p>Estoque: <b>--> . //(is_numeric($pdt_inventory) ? ($pdt_inventory >= 1 ? str_pad($pdt_inventory, 3, 0,
              // STR_PAD_LEFT) : str_pad($pdt_inventory, 3, 0, STR_PAD_LEFT)) : +100) . </b>{$PdtStockVar}</p>
            ...
            <div class='single_pdt_actions'>
                ...
                <span rel='single_pdt' callback='ProductsDoripel' callback_action='delete' class='j_delete_action_confirm icon-warning btn btn_yellow' style='display: none' id='{$pdt_id}'>Remover Produto?</span>
            </div>
        </article>";
        endforeach;
