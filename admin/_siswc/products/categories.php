...
if (DB_AUTO_TRASH):
    $Delete = new Delete;
    $Delete->exeDelete(DB_PDT_CATS_DORIPEL, "WHERE cat_title IS NULL AND cat_parent IS NULL AND cat_id >= :st", "st=1");
endif;
?>
...
<div class="dashboard_content">
    ...
    if (!$Read->getResult()):
        echo Check::erro("<span class='al_center icon-notification'>Ainda não existem categorias de produtos cadastradas {$Admin['user_name']}. Comece agora mesmo criando seu primeiro setor, e então suas categorias!</span>", E_USER_NOTICE);
    else:
        foreach ($Read->getResult() as $Sector):
            $Read->fullRead("SELECT count(pdt_id) AS total FROM " . DB_PDT_DORIPEL . " WHERE pdt_category = :sector", "sector={$Sector['cat_id']}");
            ...
            if ($Read->getResult()):
                foreach ($Read->getResult() as $Cat):
                    $Read->fullRead("SELECT count(pdt_id) AS total FROM " . DB_PDT_DORIPEL . " WHERE pdt_subcategory = :cat", "cat={$Cat['cat_id']}");
