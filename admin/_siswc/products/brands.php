...
<div class="dashboard_content">
    ...
    if (!$Read->getResult()):
        echo Check::erro("<span class='al_center icon-notification'>Ainda não existem marcas ou fabricantes cadastradas {$Admin['user_name']}. Comece agora mesmo criando suas marcas e fabricantes de produtos!</span>", E_USER_NOTICE);
    else:
        foreach ($Read->getResult() as $Brand):
            $Read->fullRead("SELECT count(pdt_id) as total FROM " . DB_PDT_DORIPEL . " WHERE pdt_brand = :brand", "brand={$Brand['brand_id']}");
            ...
            <div class='box_content'>
                ...
                <span rel='product_brand' callback='ProductsDoripel' callback_action='brand_remove' class='j_delete_action_confirm btn btn_yellow icon-warning' style='display: none;' id='{$Brand['brand_id']}'>Deletar Fabricante?</span>
            </div>
        </article>";
        endforeach;
    endif;
