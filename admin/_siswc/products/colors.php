...
<div class="dashboard_content">
    ...
    if (!$Read->getResult()):
        echo Check::erro("<span class='al_center icon-notification'>Ainda não existem padrões de cores cadastradas {$Admin['user_name']}. Comece agora mesmo criando seus padrões de cores!</span>", E_USER_NOTICE);
    else:
        foreach ($Read->getResult() as $Color):
            $Read->fullRead("SELECT count(pdt_id) as total FROM " . DB_PDT_DORIPEL . " WHERE pdt_color = :color", "color={$Color['color_id']}");
