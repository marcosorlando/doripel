...
if ($CatId):
    ...
    if ($Read->getResult()):
        ...
        $_SESSION['trigger_controll'] = Check::erro("<b>OPPSS {$Admin['user_name']}</b>, você tentou editar uma categoria que não existe ou que foi removida recentemente!", E_USER_NOTICE);
            ...
            <form class="auto_save" name="category_add" action="" method="post" enctype="multipart/form-data">
                ...
                <label class="label">
                    ...
                    <select name="cat_parent" class="jwc_pdtsection_selector">
                        ...
                        <?php
                        $Read->fullRead("SELECT cat_id, cat_title, cat_sizes FROM " . DB_PDT_CATS_DORIPEL . " WHERE cat_parent IS NULL AND cat_id != :ci ORDER BY cat_title ASC", "ci={$CatId}");
                        if ($Read->getResult()):
