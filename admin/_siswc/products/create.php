...
<header class="dashboard_header">
  ...
  <div class="dashboard_header_search">
    <a title="Criar Variação Deste Produto" href="dashboard.php?wc=products/reply&id=<?= ($pdt_parent ? $pdt_parent : $PdtId); ?>" class="btn btn_blue icon-copy">Criar
      Variação!</a>
    <a target="_blank" title="Ver no site" href="<?= BASE; ?>/movel/<?= $pdt_name; ?>" class="wc_view btn btn_green icon-eye">Ver
      no Site!</a>
  </div>
</header>
  ...
  <div class="workcontrol_imageupload_content">
    <form name="workcontrol_pdt_upload" action="" method="post" enctype="multipart/form-data">
      ...
      <div class="workcontrol_imageupload_actions">
        ...
        <span class="workcontrol_imageupload_close icon-cancel-circle btn btn_red" id="pdt_control" style="margin-right: 8px;">Fechar</span>
    ...
    <form name="pdt_size" action="" method="post">
      ...
      <div class="inputs jwc_product_stock_target">
        ...
        <div class="clear"></div>
          ...
          $CatSizes = E_PDT_SIZE;
          if ($pdt_subcategory):
              $Read->fullRead("SELECT cat_sizes FROM " . DB_PDT_CATS_DORIPEL . " WHERE cat_id = :id",
                "id={$pdt_subcategory}");
          ...
          foreach ($WcPdtSize as $Size):
              ...
              $Read->fullRead("SELECT stock_inventory, stock_sold FROM " . DB_PDT_STOCK_DORIPEL . " WHERE pdt_id = :pdt AND stock_code = :key",
                "pdt={$PdtId}&key={$Size}");
              if ($Read->getResult()):
                  echo "<label><span class='size'>{$Size}:</span><input name='{$Size}' type='number' min='0' value='{$Read->getResult()[0]['stock_inventory']}'><span class='cart'><b class='icon-cart'>" . str_pad($Read->getResult()[0]['stock_sold'],
                      2, 0, 0) . "</b></span></label>";
              else:
    ...
    <div class="box box70">
      <div class="box_content">
        ...
        <label class="label">
          ...
          <datalist id="tags">
            <?php
            $Read->fullRead("SELECT DISTINCT upper(pdt_tags) as pdt_tags FROM " . DB_PDT_DORIPEL . " WHERE pdt_tags IS NOT NULL AND pdt_tags != ''");
        ...
        <div class="label_50">
          ...
          <label class="label">
            ...
            <input type="file" value="<?= ($pdt_instrutions ? $pdt_instrutions : ''); ?>" name="pdt_instrutions"/>
          </label>
        </div>
        ...
        <div class="label_33">
          ...
          <label class="label">
            <span class="legend">Padrão/Cor:</span>
              ...
              if (!$Read->getResult()):
                  echo Check::erro("<span class='icon-warning'>Cadastre alguns padrões e/ou cores antes de começar!</span>",
                    E_USER_WARNING);
              else:
          ...
          <label class="label">
            ...
            <input type="text" name="pdt_code" placeholder="13 caracteres numéricos" title="O campo não deve conter LETRAS" maxlength="13" pattern="[0-9]+$" value="<?= ($pdt_code ? $pdt_code : ''); ?>" required/>
          </label>
        ...
        <div class="label_33">
          <label class="label">
            <span class="legend">Marca/Fabricante:</span>
              ...
              if (!$Read->getResult()):
                  echo Check::erro("<span class='icon-warning'>Cadastre algumas marcas ou fabricantes antes de começar!</span>",
                    E_USER_WARNING);
              else:
          ...
          <label class="label">
            <span class="legend">Categoria:</span>
              ...
              if (!$Read->getResult()):
                  echo Check::erro("<span class='icon-warning'>Cadastre algumas categorias de produtos antes de começar!</span>",
                    E_USER_WARNING);
              else:
                  ...
                  foreach ($Read->getResult() as $Cat):
                      ...
                      $Read->exeRead(DB_PDT_CATS_DORIPEL, "WHERE cat_parent = :id", "id={$Cat['cat_id']}");
                      if (!$Read->getResult()):
...
-->
        <span class="section icon-box-remove">DIMENSÕES DO PRODUTO POR CAIXA (1):</span>
        <div class="label_50">
          <label class="label">
            <span class="legend">Peso Bruto Em KG:</span>
            <input type="number" name="pdt_dimension_weight" value="<?= $pdt_dimension_weight; ?>" placeholder="Peso em KG:" required/>
          </label>
          <label class="label">
            <span class="legend">Comprimento Em Metros:</span>
            <input type="number" step="0.0001" name="pdt_dimension_depth" value="<?= $pdt_dimension_depth; ?>" placeholder="Comprimento em Metros:" required/>
          </label>
          <div class="clear"></div>
        </div>
        ...
        <div class="label_50">
          <label class="label">
            ...
            <input type="number" step="0.0001" name="pdt_dimension_width" value="<?= $pdt_dimension_width; ?>" placeholder="Largura em Metros:" required/>
          </label>
          <label class="label">
            <span class="legend">Altura Em Metros:</span>
            <input type="number" step="0.0001" name="pdt_dimension_heigth" value="<?= $pdt_dimension_heigth; ?>" placeholder="Altura em Metros:" required/>
          </label>
          <div class="clear"></div>
        </div>
        <span class="section icon-box-remove">DIMENSÕES DO PRODUTO POR CAIXA (2):</span>
        <div class="label_50">
          <label class="label">
            <span class="legend">Peso Bruto Em KG:</span>
            <input type="number" step="0.0001" name="pdt_dimension_weight_cx2" value="<?= $pdt_dimension_weight_cx2; ?>" placeholder="Peso em KG:" required/>
          </label>
          <label class="label">
            <span class="legend">Comprimento Em Metros:</span>
            <input type="number" step="0.0001" name="pdt_dimension_depth_cx2" value="<?= $pdt_dimension_depth_cx2; ?>" placeholder="Comprimento em Metros:" required/>
          </label>
          <div class="clear"></div>
        </div>
        ...
        <div class="label_50">
          <label class="label">
            <span class="legend">Largura Em Metros:</span>
            <input type="number" step="0.0001" name="pdt_dimension_width_cx2" value="<?= $pdt_dimension_width_cx2; ?>" placeholder="Largura em Metros:" required/>
          </label>
          <label class="label">
            <span class="legend">Altura Em Metros:</span>
            <input type="number" step="0.0001" name="pdt_dimension_heigth_cx2" value="<?= $pdt_dimension_heigth_cx2; ?>" placeholder="Altura em Metros:" required/>
          </label>
          <div class="clear"></div>
        </div>
        ...
        <div class="label_50">
          <label class="label">
            <span class="legend">Altura Em Metros:</span>
            <input type="number" step="0.0001" name="pdt_dimension_heigth_mounted" value="<?= $pdt_dimension_heigth_mounted; ?>" placeholder="Altura em Metros:" required/>
          </label>
          <label class="label">
            <span class="legend">Largura Em Metros:</span>
            <input type="number" step="0.0001" name="pdt_dimension_width_mounted" value="<?= $pdt_dimension_width_mounted; ?>" placeholder="Largura em Metros:" required/>
          </label>
        ...
        <div class="label_50">
          <label class="label">
            ...
            <input type="number" step="0.0001" name="pdt_dimension_depth_mounted" value="<?= $pdt_dimension_depth_mounted; ?>" placeholder="Profundidade em Metros:" required/>
          </label>
          <label class="label">
            <span class="legend">Peso Bruto Em KG:</span>
            <input type="number" name="pdt_dimension_weight_mounted" value="<?= $pdt_dimension_weight_mounted; ?>" placeholder="Peso em KG:" required/>
          </label>
    ...
    <div class="box box30">
      ...
      <div class="box_content">
        <label class="label">
          ...
          ?>
        <img class="pdt_scene" alt="Cena do Produto" title="Cena do Produto" src="../tim.php?src=<?= $Cena; ?>&w=1920/2&h=1152/2" default="../tim.php?src=<?= $Cena; ?>&w=1920&h=1152" />
        ...
        <!-- <label class="label">
          ...
          <div class="panel_header warning">
              ...
              <label class="label">
                  ...
                  <input type="text" name="pdt_video" placeholder="Somente ID do Vídeo" value="<?= isset($pdt_video) ? $pdt_video : ''; ?>" />
              </label>
          </div>
        ...
        <div class="wc_actions" style="text-align: center">
          <label class="label_check label_publish <?= ($pdt_status == 1 ? 'active' : ''); ?>"><input style="margin-top: -1px;" type="checkbox" value="1" name="pdt_status" <?= ($pdt_status == 1 ? 'checked' : ''); ?>>
