<?php
    ...
    if (!APP_PAGES || empty($DashboardLogin) || empty($Admin) || $Admin['user_level'] < LEVEL_WC_PAGES) {
        Check::accessBlocked();
    }
...
<header class="dashboard_header">
	...
	<div class="dashboard_header_search">
		<a title="Nova Página" href="dashboard.php?wc=pages/create" class="btn btn_green icon-plus">Adicionar</a>
...
<div class="dashboard_content">
    <?php
        ...
        $Read->exeRead(
            DB_PAGES,
            'ORDER BY page_order ASC, page_title ASC, created_at DESC LIMIT :limit OFFSET :offset',
        ...
        if (!$Read->getResult()) {
            ...
            foreach ($Read->getResult() as $PAGE) {
                ...
                $page_cover = empty($page_cover)
                    ? BASE . '/tim.php?src=admin/_img/no_image.jpg&w=' . (IMAGE_W / 4) . '&h=' . (IMAGE_H / 4)
                    : BASE . sprintf('/tim.php?src=uploads/%s&w=', $page_cover) . (IMAGE_W / 4) . '&h=' . (IMAGE_H / 4);
